...
namespace FDILink {
    ahrsBringup::ahrsBringup() : frist_sn_(false), serial_timeout_(20) {
        ...
        mag_pose_pub_ = nh_.advertise<geometry_msgs::Pose2D>("/mag_pose_2d", 10);
        //setp up serial
        try {
    ...
    void ahrsBringup::processLoop() {
        ...
        while (ros::ok()) {
            ...
            if (head_type[0] == TYPE_IMU && check_len[0] != IMU_LEN) {
                ...
                continue;
            } else if (head_type[0] == TYPE_GROUND || head_type[0] == 0x50) // Unknown data to prevent recording failure
            {
                uint8_t ground_sn[1];
            ...
            if (head_type[0] == TYPE_IMU) {
                ...
                uint16_t head_crc16 = insgps_frame_.frame.header.header_crc16_l +
                                      ((uint16_t) insgps_frame_.frame.header.header_crc16_h << 8);
            ...
            if (head_type[0] == TYPE_AHRS) {
                ...
                } else if (device_type_ == 1)    //Coordinate transformation under the ROS standard of imu single product
                {
                ...
                } else if (device_type_ == 1) {   //Coordinate transformation of car and imu single product under ROS standard
                    magx = -imu_frame_.frame.data.data_pack.magnetometer_x;
                    magy = imu_frame_.frame.data.data_pack.magnetometer_y;
                    magz = imu_frame_.frame.data.data_pack.magnetometer_z;
    ...
    void
    ahrsBringup::magCalculateYaw(double roll, double pitch, double &magyaw, double magx, double magy, double magz) {
